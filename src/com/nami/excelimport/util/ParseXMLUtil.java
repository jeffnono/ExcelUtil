...
public class ParseXMLUtil {
	 ...
	 */
	public static ExcelStruct parseImportStruct(InputStream xmlFile) throws Exception {
		if (xmlFile == null) {
			return null;
		}
		...
		// InputStream is =
		// ParseXMLUtil.class.getClassLoader().getResourceAsStream(xmlFile);
		if (is == null) {
	...
	private static void simpleParseOnceImport(ExcelStruct excelStruct, List<Element> onceList) {
		...
		// 获取CDATA区内的内容
		List<Element> columns = onceElem.getChildren("column");
		List<ImportCellDesc> onceImportCells = changeCDATAToList(excelStruct, columns);
	...
	private static void simpleParseRepeatImport(ExcelStruct excelStruct, List repeatList) {
		...
		// 获取CDATA区内的内容
		List<Element> columns = repElem.getChildren("column");
		List<ImportCellDesc> repeatImportCells = changeCDATAToList(excelStruct, columns);
	 ...
	 */
	private static List<ImportCellDesc> changeCDATAToList(ExcelStruct excelStruct, List<Element> columns) {
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			String sourceColumn = column.getAttributeValue("sourceColumn");
			if (StringUtil.isEmpty(sourceColumn)) {
				continue;
			}
			String targetColumn = column.getAttributeValue("targetColumn");
			if (StringUtil.isEmpty(targetColumn)) {
				continue;
			}

			ImportCellDesc cellDesc = new ImportCellDesc();
			cellDesc.setCellRef(sourceColumn);
			String targetTable = column.getDocument().getRootElement().getAttributeValue("targetTable");
			if (StringUtil.isNotEmpty(targetTable)) {
				cellDesc.setTargetTable(targetTable);
			}
			cellDesc.setFieldName(targetColumn);
			if (excelStruct != null) {
			...
			if (StringUtil.isNotEmpty(length)) {
				cellDesc.setLength(length);
			}
			String startIndex = column.getAttributeValue("startIndex");
			if (StringUtil.isNotEmpty(startIndex)) {
				cellDesc.setStartIndex(startIndex);
			}
