...
public class ExcelDataReader {
	 ...
	 *          需要组装的数据
	 */
	private static int readRepeatImportData(List<ImportCellDesc> repeatImportCells, Sheet sheet, FormulaEvaluator evaluator, String endCode,
			ExcelData excelData) {
		int repeatCount = 0;
		...
		for (int i = beginRow; i <= sheet.getLastRowNum(); i++) {
			...
			for (ImportCellDesc desc : repeatImportCells) {
				...
				newDesc.setTargetTable(desc.getTargetTable());
				...
				if (StringUtil.isNotEmpty(desc.getLength())) {
					int length = Integer.valueOf(desc.getLength());
					int startIndex = 0;
					String index = desc.getStartIndex();
					if (StringUtil.isNotEmpty(index)) {
						startIndex = Integer.valueOf(index);
					}
					if (fieldValue.length() >= length) {
						fieldValue = fieldValue.substring(startIndex, length);
					}
				}
	...
	private static String getCellValue(Cell cell) {
		...
		boolean isMergeRegion = isMergedRegion(sheet, row, column);
		if (isMergeRegion) {
			return getMergedRegionValue(cell);
		}
		return getCommonCellValue(cell);
	}

	private static String getMergedRegionValue(Cell cell) {
		Sheet sheet = cell.getSheet();
		int row = cell.getRowIndex();
		int column = cell.getColumnIndex();
		...
		for (int i = 0; i < sheetMergeCount; i++) {
			...
			if (row >= firstRow && row <= lastRow) {
				if (column >= firstColumn && column <= lastColumn) {
					...
					return getCommonCellValue(fCell);
				}
			}
		}
		return "";
	...
	private static String getCommonCellValue(Cell cell) {
		String fieldValue = "";
		if (cell != null) {
			...
			case Cell.CELL_TYPE_NUMERIC: // 数值型
				...
				fieldValue = df.format(cell.getNumericCellValue());
				break;
