...
public class ValidateUtil {
	...
	private static void validateRepeateData(ExcelData excelData) {
		...
		List<Map<String, ImportCellDesc>> invalidData = new ArrayList<Map<String, ImportCellDesc>>();
		for (Map<String, ImportCellDesc> map : repeatData) {
			...
			for (String key : keys) {
				...
				if (excelData.getErrorMsgList()!=null && excelData.getErrorMsgList().containsKey(cellDesc.getCellRef())){
					System.out.println(cellDesc.getCellRef() +"不合法");
					invalidData.add(map);
				}
			}
		}
		repeatData.removeAll(invalidData);
	}
